...
4. uco runtime shortcuts/generated commands control an already-running Editor/server through REST. A single REST surface serves this agent-to-uco-to-server path.
...
The tool server is the Node.js uco server (`uco/src/server/`, shipped compiled in the package): REST on the agent side, a raw-WebSocket bridge to the Unity plugin on the Editor side. Raw official `unity mcp`/Pipeline capabilities are a separate official domain.
...
`dist/` is generated by TypeScript build and is not an authored documentation target. 
