...
import type { ProgressBar } from './ui.js';

export interface UnityHubOutput {
...
export interface UnityHubProcessAdapter {
  execFileSync: typeof execFileSync;
  execSync: typeof execSync;
  spawn: typeof spawn;
}

export interface UnityHubOperationOptions {
  /** Explicit output sink; lifecycle JSON sessions use the silent sink. */
  output?: UnityHubOutput;
  /** @internal Injectable process seam for deterministic production-path tests. */
  processAdapter?: Partial<UnityHubProcessAdapter>;
  /** Throw inventory execution failures instead of preserving legacy `[]`. */
  strictInventory?: boolean;
}

function createSilentSpinner(initialText: string): Spinner {
  ...
  const spinner = {
    ...
    set color(_value: string) { /* no output */ },
  } as unknown as Spinner;
  return spinner;
}

const defaultUnityHubOutput: UnityHubOutput = {
  verbose: ui.verbose,
  info: ui.info,
  warn: ui.warn,
  success: ui.success,
  ...
  createProgressBar: ui.createProgressBar,
};

/** A per-operation sink for JSON/library callers; it never touches process streams. */
export const silentUnityHubOutput: UnityHubOutput = {
  verbose: () => undefined,
  info: () => undefined,
  warn: () => undefined,
  success: () => undefined,
  label: () => undefined,
  startSpinner: createSilentSpinner,
  createProgressBar: () => ({
    update: () => undefined,
    complete: () => undefined,
    fail: () => undefined,
...
function outputFor(options: UnityHubOperationOptions): UnityHubOutput {
  return options.output ?? defaultUnityHubOutput;
}
 ...
 */
export function findUnityHub(options: UnityHubOperationOptions = {}): string | null {
  const startedAt = Date.now();
  const output = outputFor(options);
  const plat = platform();
  ...
  for (const candidate of candidates) {
    ...
    output.verbose(`Checking Unity Hub candidate: ${candidate}`);
    if (candidate && fs.existsSync(candidate)) {
      output.verbose(`Found Unity Hub at: ${candidate} (${Date.now() - candidateStartedAt}ms for candidate, ${Date.now() - startedAt}ms total)`);
      return candidate;
    }
    output.verbose(`Unity Hub candidate missing: ${candidate} (${Date.now() - candidateStartedAt}ms)`);
  }

  output.verbose(`Unity Hub not found in any candidate location (${Date.now() - startedAt}ms total)`);
  return null;
}
 ...
 */
export async function installUnityHub(options: UnityHubOperationOptions = {}): Promise<string> {
  const plat = platform();
  const output = outputFor(options);
  const runFileSync = options.processAdapter?.execFileSync ?? execFileSync;
  const runSync = options.processAdapter?.execSync ?? execSync;
  ...
  switch (plat) {
    case 'win32': {
      ...
      const spinner = output.startSpinner('Downloading Unity Hub installer...');
      try {
      ...
      output.info('Installing Unity Hub silently (may require administrator privileges)...');
      try {
        runFileSync(installerPath, ['/S'], {
          timeout: 300000,
          stdio: options.output === undefined ? 'inherit' : 'pipe',
        });
      } finally {
        try { fs.unlinkSync(installerPath); } catch { /* ignore */ }
      }
      const hubPath = findUnityHub(options);
      if (!hubPath) {
    ...
    case 'darwin': {
      ...
      spinner.success('Unity Hub installer downloaded');
      const installSpinner = output.startSpinner('Installing Unity Hub...');
      try {
        const mountOutput = runSync(`hdiutil attach -nobrowse -noverify "${dmgPath}"`, { encoding: 'utf-8' });
        ...
        try {
          runSync(`cp -R "${mountPoint}/Unity Hub.app" /Applications/`, {
            stdio: options.output === undefined ? 'inherit' : 'pipe',
          });
        } finally {
          runSync(`hdiutil detach "${mountPoint}" -quiet`, { stdio: 'ignore' });
        }
    ...
    case 'linux': {
      ...
      const spinner = output.startSpinner('Downloading Unity Hub AppImage...');
 ...
 */
export async function ensureUnityHub(options: UnityHubOperationOptions = {}): Promise<string> {
  const startedAt = Date.now();
  const output = outputFor(options);
  const hubPath = findUnityHub(options);
  if (hubPath) {
    output.verbose(`ensureUnityHub reused existing installation in ${Date.now() - startedAt}ms`);
    return hubPath;
  }

  output.warn('Unity Hub not found. Installing automatically...');
  const installedHubPath = await installUnityHub(options);
  output.verbose(`ensureUnityHub installed Unity Hub in ${Date.now() - startedAt}ms`);
...
export function listInstalledEditors(
  hubPath: string,
  options: UnityHubOperationOptions = {},
): InstalledEditor[] {
  const output = outputFor(options);
  const spinner = output.startSpinner('Listing installed editors...');
  const runFileSync = options.processAdapter?.execFileSync ?? execFileSync;
  const startedAt = Date.now();
  try {
    output.verbose(`listInstalledEditors invoking Unity Hub CLI: ${hubPath} -- --headless editors --installed`);
    ...
    const commandOutput = runFileSync(hubPath, ['--', '--headless', 'editors', '--installed'], {
      encoding: 'utf-8',
      timeout: 120000,
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    output.verbose(`listInstalledEditors Unity Hub CLI returned in ${Date.now() - execStartedAt}ms`);
    ...
    if (options.strictInventory && editors.length === 0 && commandOutput.trim()) {
      throw new Error('Unity Hub returned non-empty installed-editor output that could not be parsed.');
    }
    output.verbose(`listInstalledEditors parsed ${editors.length} editor(s) in ${Date.now() - parseStartedAt}ms`);

    spinner.success(`Found ${editors.length} installed editor${editors.length !== 1 ? 's' : ''}`);
    output.verbose(`listInstalledEditors completed in ${Date.now() - startedAt}ms`);
    return editors;
  } catch (err) {
    spinner.error(`Failed to list installed editors: ${formatExecError(err)}`);
    output.verbose(`listInstalledEditors failed after ${Date.now() - startedAt}ms`);
    if (options.strictInventory) {
      throw new Error(`Failed to list installed Unity editors: ${formatExecError(err)}`, {
        cause: err,
      });
    }
    return [];
...
export function listAvailableReleases(
  ...
  const output = outputFor(options);
  const spinner = output.startSpinner('Fetching available releases...');
  const runFileSync = options.processAdapter?.execFileSync ?? execFileSync;
  try {
    // Same stderr suppression rationale as listInstalledEditors above.
    const commandOutput = runFileSync(hubPath, ['--', '--headless', 'editors', '--releases'], {
      encoding: 'utf-8',
      timeout: 120000,
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    ...
    if (options.strictInventory && releases.length === 0 && commandOutput.trim()) {
      throw new Error('Unity Hub returned non-empty release output that could not be parsed.');
    }
    ...
    if (options.strictInventory) {
      throw new Error(`Failed to list available Unity releases: ${formatExecError(err)}`, {
        cause: err,
      });
    }
    return [];
 ...
 */
function runHubInstallWithProgress(
  hubPath: string,
  args: string[],
  version: string,
  options: UnityHubOperationOptions,
  ...
  return new Promise((resolve, reject) => {
    const output = outputFor(options);
    const progress = output.createProgressBar();
    let activeSpinner: Spinner | null = output.startSpinner(`Preparing Unity Editor ${version}...`);
    let isDownloading = false;

    const spawnProcess = options.processAdapter?.spawn ?? spawn;
    const proc = spawnProcess(hubPath, args, { stdio: ['ignore', 'pipe', 'pipe'] });
    let stderr = '';
    ...
    function startPhaseSpinner(status: string): void {
      ...
      activeSpinner = output.startSpinner(`${status}...`);
    }
...
export async function installEditor(
  hubPath: string,
  version: string,
  prefetchedReleases?: AvailableRelease[],
  options: UnityHubOperationOptions = {},
): Promise<void> {
  // Use pre-fetched releases when available to avoid a duplicate Hub CLI call
  const output = outputFor(options);
  const releases = prefetchedReleases ?? listAvailableReleases(hubPath, options);
  const isPromoted = releases.some(r => r.version === version);

  const baseArgs = ['--', '--headless', 'install', '--version', version];

  if (isPromoted) {
    await runHubInstallWithProgress(hubPath, baseArgs, version, options);
    return;
  }

  // Version not in releases — resolve changeset
  const changesetSpinner = output.startSpinner(`Resolving changeset for ${version}...`);
  ...
  await runHubInstallWithProgress(hubPath, [...baseArgs, '--changeset', changeset], version, options);
}
...
export function createProject(
  ...
  output.info(`Creating Unity project at: ${projectPath}`);
  output.label('Unity Editor', `${editor.version} (${editorExe})`);
  try {
    runFileSync(editorExe, args, {
      encoding: 'utf-8',
      timeout: 120000,
      stdio: options.output === undefined ? 'inherit' : ['ignore', 'pipe', 'pipe'],
    });
