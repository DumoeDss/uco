...
export interface EditorLocatorDependencies {
  selectSession?: UnityLifecycleSessionSelector;
  readCachedEditorPath?: (version: string | undefined) => string | null;
  writeCachedEditorPath?: (version: string | undefined, editorPath: string) => void;
  findEditorPathByCommonLocations?: (version: string | undefined) => string | null;
}

/**
 * Find the Unity Editor binary path for a specific version.
 * Uses the selected lifecycle backend to locate installed editors. In auto
 * mode, Unity Hub remains the absence-only fallback.
 */
export async function findEditorPath(
  version?: string,
  dependencies: EditorLocatorDependencies = {},
): Promise<string | null> {
  const startedAt = Date.now();
  verbose(`findEditorPath start (version=${version ?? 'auto'})`);
  const selectSession = dependencies.selectSession ?? selectUnityLifecycleSession;
  const readCache = dependencies.readCachedEditorPath ?? readCachedEditorPath;
  const writeCache = dependencies.writeCachedEditorPath ?? writeCachedEditorPath;
  const findCommon = dependencies.findEditorPathByCommonLocations
    ?? findEditorPathByCommonLocations;
  ...
  if (version) {
    ...
    const fastResult = findCommon(version);
    verbose(`findEditorPath fast path completed in ${Date.now() - fastStartedAt}ms (${fastResult ? 'hit' : 'miss'})`);
    if (fastResult) {
      writeCache(version, fastResult);
      verbose(`findEditorPath resolved via common locations in ${Date.now() - startedAt}ms`);
      return fastResult;
    }
  }

  // Slow path: select one lifecycle backend and keep it sticky for inventory.
  const selectionStartedAt = Date.now();
  // This utility is also a library boundary; backend progress must never leak
  // directly to process streams. CLI callers surface their own progress.
  const session = selectSession({ silent: true });
  verbose(
    `findEditorPath selected ${session.backend} (${session.decision.reason}) in ${Date.now() - selectionStartedAt}ms`,
  );
  const listStartedAt = Date.now();
  const editors = await session.listInstalledEditors();
  verbose(
    `findEditorPath ${session.backend} inventory completed in ${Date.now() - listStartedAt}ms (count=${editors.length})`,
  );
  if (editors.length === 0) {
    const fallbackStartedAt = Date.now();
    const fallback = findCommon(version);
    ...
    if (fallback) writeCache(version, fallback);
    return fallback;
  }
