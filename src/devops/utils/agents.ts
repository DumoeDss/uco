...
export const agentRegistry: readonly AgentDefinition[] = [
  ...
  // ── Windsurf ────────────────────────────────────────────────
  {
    ...
    configPathDisplay: '.windsurf/mcp.json',
    configFormat: 'json',
    bodyPath: 'mcpServers',
    getConfigPath: (p) => path.join(p, '.windsurf', 'mcp.json'),
    getStdioProps: (serverPath, port, timeout, auth, token) => ({
      type: 'stdio',
      command: serverPath,
      args: stdioArgs(port, timeout, auth, token),
...
/**
 * Pure merge core of {@link writeJsonAgentConfig}: reconciles the managed
 * server entry inside a parsed config root (surgical — unrelated keys are
 * preserved) and returns the root that would be serialized. Callers that need
 * byte-identical skip semantics (notably `uco update`) can run this over the
 * parsed current file and compare the serialization before writing.
 */
export function mergeJsonAgentConfigRoot(
...
export function writeJsonAgentConfig(
  ...
  mergeJsonAgentConfigRoot(root, bodyPath, serverName, props, removeKeys);
...
/**
 * Pure merge core of {@link writeTomlAgentConfig}: reconciles the managed
 * section inside the config's line array and returns the lines that would be
 * written. Byte-identical skip semantics mirror
 * {@link mergeJsonAgentConfigRoot}.
 */
export function mergeTomlAgentConfigLines(
  lines: string[],
  bodyPath: string,
  serverName: string,
  props: Record<string, unknown>,
  ...
  const sectionIdx = merged.findIndex(
  ...
  if (sectionIdx >= 0) {
    ...
    while (endIdx < merged.length && !merged[endIdx].trim().startsWith('[')) {
      endIdx++;
    }
    // Replace section
    merged.splice(sectionIdx, endIdx - sectionIdx, ...tomlLines);
  } else {
    // Append section
    if (merged.length > 0 && merged[merged.length - 1].trim() !== '') {
      merged.push('');
    }
    merged.push(...tomlLines);
  }
  return merged;
}
...
export function writeTomlAgentConfig(
  ...
  const merged = mergeTomlAgentConfigLines(lines, bodyPath, serverName, props, removeKeys);
  fs.writeFileSync(configPath, merged.join('\n') + '\n');
}
