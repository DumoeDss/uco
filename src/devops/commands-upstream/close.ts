...
export const closeCommand = new Command('close')
  ...
  .option('--timeout-seconds <seconds>', `Polite-quit timeout in seconds (canonical; default: ${DEFAULT_TIMEOUT_SECONDS})`)
  .option('--timeout <seconds>', 'Deprecated alias for --timeout-seconds; value is seconds', String(DEFAULT_TIMEOUT_SECONDS))
  ...
  .action(async (positionalPath: string | undefined, options: CloseOptions) => {
    ...
    const timeoutRaw = options.timeoutSeconds ?? options.timeout;
    const timeoutSeconds = parseTimeoutSeconds(timeoutRaw);
    if (timeoutSeconds === null) {
      ui.error(`Invalid timeout value: "${timeoutRaw}". Must be a positive integer (seconds).`);
      process.exit(1);
    }
