...
export const bootstrapLocalCommand = new Command('bootstrap-local')
  ...
  .action(async (positionalPath: string | undefined, options: BootstrapLocalOptions) => {
    ...
    // uco 0.3.0 renamed the config file; write the new name, but honor a legacy
    // file when it is the only one present (the plugin migrates it on next start).
    const configPath = path.join(projectPath, 'UserSettings',
      fs.existsSync(path.join(projectPath, 'UserSettings', 'uco-config.json'))
        ? 'uco-config.json'
