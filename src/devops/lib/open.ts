...
import { redactSensitiveText, redactSensitiveValue } from '../../util/redaction.js';
import type {
  OpenEnvInputs,
...
export async function openProject(
  ...
  try {
    ...
    if (options.noConnect === true && options.startServer === true) {
      throw new Error('--no-connect cannot be combined with COCli-owned --start-server true.');
    }
    if (options.transport === 'stdio' && options.startServer === true) {
      throw new Error('COCli-owned --start-server true requires streamableHttp transport.');
    }

    // An existing Editor cannot receive a new launch environment. An explicit
    // owned-server request therefore cannot be fulfilled by this invocation;
    // fail honestly instead of starting a bridge that the Editor cannot use.
    const existingProcess = findUnityProcess(projectPath);
    if (existingProcess) {
      if (options.startServer === true) {
        throw new Error(
          `Unity is already running with this project (PID: ${existingProcess.pid}). ` +
          'Close it and reopen with --start-server true; the running Editor cannot receive ' +
          'the verified connection environment from a new uco launch.',
        );
      }
      warnings.push(
    ...
    // the plugin WebSocket connection. Tell the plugin not to start a duplicate.
    if (effectiveOptions.startServer === true) {
      emitProgress(options.onProgress, {
        phase: 'starting-node-server',
        message: 'Starting the local Node bridge before Unity',
      });
      const ownedServer = await prepareOwnedNodeServer({
        projectPath,
        ...(effectiveOptions.url ? { url: effectiveOptions.url } : {}),
        ...(effectiveOptions.token ? { token: effectiveOptions.token } : {}),
        ...(effectiveOptions.auth ? { auth: effectiveOptions.auth } : {}),
      });
      effectiveOptions = {
        ...effectiveOptions,
        url: ownedServer.baseUrl,
        token: ownedServer.token,
        auth: ownedServer.authorization,
        startServer: false,
      };
      env = buildOpenEnv(effectiveOptions);
      emitProgress(options.onProgress, {
        phase: 'node-server-ready',
        message: ownedServer.alreadyRunning
          ? 'Local Node bridge was already ready'
          : 'Local Node bridge is listening and authenticated',
        alreadyRunning: ownedServer.alreadyRunning,
        pid: ownedServer.pid,
      });
    }
    ...
    const withoutExplicitToken = options.token
      ? originalMessage.split(options.token).join('[REDACTED]')
      : originalMessage;
    const errorObj = new Error(redactSensitiveText(withoutExplicitToken));
    return {
      kind: 'failure',
      success: false,
