...
export interface AgentMcpProps {
  /** Absolute path of the agent's MCP config file for this project. */
  configPath: string;
  /** Managed-entry properties to reconcile into the config. */
  props: Record<string, unknown>;
  /** Stale keys the writer strips from the managed entry. */
  removeKeys: string[];
}

/**
 * Compute the managed MCP server entry an agent's config should carry for a
 * project, from the project's live `UserSettings/uco-config.json`
 * (the same resolution `setupMcp` performs). Shared by `setupMcp` and
 * `uco update`'s reconciliation so both always write the same entry.
 */
export function computeAgentMcpProps(
  ...
  const token = overrides.token ?? fromConfig.token ?? '';
  ...
  return { configPath, props, removeKeys: agent.httpRemoveKeys };
}
...
export async function setupMcp(opts: SetupMcpOptions): Promise<SetupMcpResult> {
  ...
  try {
    ...
    if (transport === 'stdio') {
      ...
      const computed = computeAgentMcpProps(agent, projectPath, {
        ...(opts.url ? { url: opts.url } : {}),
        ...(opts.token ? { token: opts.token } : {}),
      });
      props = computed.props;
      removeKeys = computed.removeKeys;
    }
